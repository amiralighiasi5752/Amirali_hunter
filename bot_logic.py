...
def process_new_signal(item):
    ...
    signal_price = float(daily_df.iloc[-1]["close"])
...
def check_waiting_entry(item, open_count):
    ...
    if open_count >= MAX_CONCURRENT_TRADES:
        # به سقف تعداد پوزیشن‌های هم‌زمان رسیدیم؛ فعلاً وارد این معامله نمی‌شیم
        # حتی اگه قیمت به نقطه ورود برسه، تا یکی از پوزیشن‌های باز بسته نشه صبر می‌کنیم
    ...
    hit = hourly_df[hourly_df["low"] <= entry_target]
    if len(hit) > 0:
        entry_time = hit.iloc[0]["open_time"]
        item["status"] = "open"
