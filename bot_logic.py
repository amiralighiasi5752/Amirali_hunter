...
def check_open_trade(item):
    ...
    for _, row in hourly_df.iterrows():
        # خروج زودهنگام: اگه ساختار حمایتی بشکنه قبل از رسیدن به حد ضرر رسمی،
        # زودتر خارج می‌شیم چون این خودش یه هشدار جدی‌تره
        if recent_low is not None and row["close"] < recent_low and row["close"] > stop_loss:
            item["status"] = "closed"
            item["outcome"] = "STRUCTURE_BREAK"
            ret = (row["close"] - item["entry_target"]) / item["entry_target"] * 100
            item["return_pct"] = round(ret, 1)
            send_message(
                f"🟠 {symbol}: ساختار حمایتی شکست (زیر {recent_low:.6g} بسته شد) — "
                f"زودتر از حد ضرر رسمی خارج شدیم. بازدهی: {ret:.1f}%"
            )
            return item
