...
def main():
    ...
    invalidated = [i for i in items if i.get("status") == "invalidated"]
    ...
    print(f"  باطل شد (شکست ساختار حمایتی): {len(invalidated)}")
    ...
    wins = [i for i in closed if i["return_pct"] > 0]
    losses = [i for i in closed if i["return_pct"] <= 0]
    ...
    print(f"  TAKE_PROFIT: {sum(1 for i in closed if i['outcome']=='TAKE_PROFIT')}")
    print(f"  STOP_LOSS: {sum(1 for i in closed if i['outcome']=='STOP_LOSS')}")
    print(f"  STRUCTURE_BREAK (خروج زودهنگام): {sum(1 for i in closed if i['outcome']=='STRUCTURE_BREAK')}")
    print(f"وین‌ریت (بر اساس بازدهی مثبت/منفی): {win_rate:.1f}%")
